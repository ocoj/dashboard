...
export function GroupUsersTableColumns(
	t: (key: string) => string,
): ColumnDef<User>[] {
	return [
		{
		...
		},
		{
			...
			header: ({ column }) => {
				return (
					<DataTableHeader column={column}>{t("blockUser")}</DataTableHeader>
				);
			},
			sortingFn: "text",
		...
		},
		{
			...
			header: ({ column }) => {
				return (
					<DataTableHeader column={column}>{t("lastLogin")}</DataTableHeader>
				);
			},
			sortingFn: "text",
			cell: ({ row }) => (
