...
## ⚠️ 文件存储规范（所有 Agent 必须遵守）

本项目严格区分 **公开文档** 和 **内部文档**，放错目录会导致敏感信息泄露到 GitHub。

| 目录 | 用途 | 上传 GitHub？ |
|------|------|:---:|
| **`docs/`** | 公开项目文档（架构说明、上下文、截图等） | ✅ 是 |
| **`internal/`** | 内部文档（部署凭据、会话记录、提示词模板、含 IP/密码的文件） | ❌ 否（`.gitignore` 排除） |

### 红线规则
- **禁止**在 `docs/` 下存放含密码、Token、内部 IP、SSH 信息的文件
- **禁止**在 `src/`、`public/` 等源码目录下存放文档文件
- **禁止**在项目根目录创建新的一级文档目录 — 公开的放 `docs/`，私有的放 `internal/`
- 新创建文档前，先判断内容是否含敏感信息，再选择正确的目录
